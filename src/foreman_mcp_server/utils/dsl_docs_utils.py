...
def _format_method_documentation(method: Dict[str, Any], is_property: bool = False) -> List[str]:
...
def _format_class_documentation(class_name: str, class_desc: Dict[str, Any]) -> List[str]:
...
def parse_dsl_docs(cache_content: str) -> str:
  ...
  erb_templates = [
    ...
    ("<% code -%>", "Executes code, but does not insert a value, trims the following line break"),
    ("<%= expression -%>", "Inserts the value of an expression, trims the following line break"),
    ("<%# comment -%>", "Comment, removed from the final output, trims the following line break"),
  ]
...
def get_dsldocs_path(cache_dir, cache_file) -> str:
  ...
  dsldoc_cache_dir = os.path.join(cache_dir, 'dsldoc')
  if not os.path.exists(dsldoc_cache_dir):
    os.makedirs(dsldoc_cache_dir, exist_ok=True)

  return os.path.join(dsldoc_cache_dir, cache_file)
...
async def read_dsl_docs_from_markdown(cache_dir, cache_file) -> str:
  ...
  try:
    async with aiofiles.open(dsldoc_cache_file, 'r') as file:
      return await file.read()
  except FileNotFoundError:
    raise FileNotFoundError(f"DSL documentation file '{dsldoc_cache_file}' not found.")
  except Exception as e:
